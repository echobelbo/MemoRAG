...
- 请用中文回答。""",
    ...
    "ner_mem": "请对以下文本进行实体识别任务, 并找出每个实体的共现实体，且去除其中的同义词，近义词实体，输出结果符合如下格式：{{\"实体1\": [\"实体a\", \"实体b\",...,\"实体c\"],\"实体2\": [\"实体d\", \"实体e\",...,\"实体f\"],...\"实体n\": [\"实体g\", \"实体h\",...,\"实体i\"]}}\n\n{context}",
    "ner_qa": "请对以下查询文本进行实体识别任务, 且使得结果符合查询任务需求, 输出结果符合如下格式: 实体1, 实体2, ..., 实体n\n\n{question}",
    "keyword_refine": "请根据提供的关键词对查询文本进行优化，尽可能利用关键词来丰富查询文本中所包含的信息\n查询文本:{query}\n关键词{key_words}\n请仅生成1个查询,且生成结果仍应保持原本查询的语义。",
    "rel": "请评估以下文本与问题的相关性，返回0-1之间的分数。问题：{query}\n文本：{chunk}\n只返回分数，不要其他内容。",
    "claim_gen": "从以下文本中提取与问题相关的关键声明：\n问题：{query}\n文本：{group_text}\n请提取3-5个关键声明，每个声明应该：直接回答问题的某个方面, 具体且有信息量, 避免重复信息, 仅返回声明列表,每行一个声明。",
